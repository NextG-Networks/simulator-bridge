...
/*
 * 10-MINUTE NOVEL CHAOS SCENARIO — RL EVALUATION
 * Simulates 600s (10 min). More severe and rapid out-of-distribution events.
 */
...
NS_LOG_COMPONENT_DEFINE("Eval_Novel_10MIN");

// --- 10 MINUTE RUNTIME (600 seconds) ---
static GlobalValue g_simTime("simTime", "Simulation time (s)", DoubleValue(600.0), MakeDoubleChecker<double>(1.0, 36000.0));
...
static void SampleAll(const NodeContainer &ueNodes, const NetDeviceContainer &ueDevs, Ptr<Node> gnbNode, double covRadius, ApplicationContainer sinkApps, double periodSec) {
  ...
  if (g_enableCsvLogging) {
    if (!headerDone) {
      f.open("sim_timeseries_eval_10min.csv", std::ios::out | std::ios::trunc);
...
static void TriggerBlockageEvent(NodeContainer ues, double durationS) {
    ...
    if (ueDev && ueDev->GetPhy()) {
        ...
        phy->SetNoiseFigure(originalNf + 80.0); // NOVEL: +80dB (Severe Blockage)
        g_activeBlockage = true; g_blockageImsi = ueDev->GetImsi();
        NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "s: [NOVEL] SEVERE Blockage on UE " << g_blockageImsi << " for " << durationS << "s");
...
static void TriggerTrafficSpikeEvent(NodeContainer remoteHosts, double durationS) {
    ...
    if (!onOffApps.empty()) {
        ...
        std::string spikeRate = "800Mbps"; // NOVEL: Much higher spike
        onOffApp->SetAttribute("DataRate", DataRateValue(DataRate(spikeRate)));
        g_activeTrafficSpike = true; g_trafficSpikeRhIdx = rhIdx;
        NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "s: [NOVEL] MASSIVE Spike " << spikeRate << " on RH " << rhIdx << " for " << durationS << "s");
...
static void TriggerMcsDropEvent(Ptr<Node> gnb, double durationS) {
    int targetMcs = 2 + (rand() % 4); // NOVEL: Very low MCS drop (2 to 5)
    ChangeMcs(gnb, targetMcs);
    g_activeMcsEvent = "MCS_" + std::to_string(targetMcs);
    NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "s: [NOVEL] SEVERE MCS Drop to " << targetMcs << " for " << durationS << "s");
    Simulator::Schedule(Seconds(durationS), [gnb]() { ChangeMcs(gnb, -1); g_activeMcsEvent = "None"; });
}

// --- Rapid-Fire Scheduler ---
static void ScheduleRandomEvent(Ptr<UniformRandomVariable> rng, NodeContainer ues, Ptr<Node> gnb, NodeContainer rh) {
    if (Simulator::Now().GetSeconds() >= g_simStopTime - 30.0) return; // Stop scheduling near end
    
    // NOVEL: Shorter events, much shorter cooldowns (Aggressive)
    ...
    switch(eventType) {
        ...
        case 3: TriggerBlockageEvent(ues, duration); TriggerTrafficSpikeEvent(rh, duration); break; // Compound
    }
...
int main (int argc, char** argv) {
  ...
  Simulator::Schedule(Seconds(10.0), &ScheduleRandomEvent, rng, ue, gnb.Get(0), rh);

  NS_LOG_UNCOND("Starting 10-Min NOVEL EVALUATION Scenario");
